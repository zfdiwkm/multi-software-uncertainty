...
from InnerEye.ML.utils.io_util import ImageWithHeader, load_dicom_series_and_save, load_nifti_image, \
    reverse_tuple_float3, store_as_ubyte_nifti, store_posteriors_as_nifti
...
def score_image(args: ScorePipelineConfig) -> Path:
    ...
    if config.dataset_expected_spacing_xyz:
        for image in images:
            spacing_xyz = reverse_tuple_float3(image.header.spacing)
            if not is_spacing_valid(spacing_xyz, config.dataset_expected_spacing_xyz):
                raise ValueError(f'Input image has spacing {spacing_xyz} '
                                 f'but expected {config.dataset_expected_spacing_xyz}')
    ...
    photo_norm = PhotometricNormalization(config_args=config)
    photo_norm_images = [photo_norm.transform(image.image) for image in images]
    
    # Get the full inference result, not just the segmentation
    inference_result = inference_pipeline.predict_and_post_process_whole_image(
        image_channels=np.array(photo_norm_images),
        voxel_spacing_mm=images[0].header.spacing
    )

    segmentation_file_name = model_folder / args.result_image_name
    result_dst = store_as_ubyte_nifti(inference_result.segmentation, images[0].header, segmentation_file_name)
    ...
    if not is_offline_run_context(run_context):
        ...
        # Also upload the posterior files
        for posterior_file in posterior_files:
            run_context.upload_file(posterior_file.name, str(posterior_file))
