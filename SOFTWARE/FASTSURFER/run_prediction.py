...
class RunModelOnData:
    ...
    _pool: Executor
    save_hippo_probs: bool
    _hippo_probs: np.ndarray | None
    _left_hippo_probs: np.ndarray | None
    _right_hippo_probs: np.ndarray | None
    _left_hippo_raw: np.ndarray | None
    _right_hippo_raw: np.ndarray | None
    ...
    def __init__(
        ...
        self.save_hippo_probs = save_hippo_probs
        self._hippo_probs = None
        self._left_hippo_probs = None
        self._right_hippo_probs = None
        self._left_hippo_raw = None
        self._right_hippo_raw = None
    ...
    def get_prediction(
        ...
        if self.save_hippo_probs:
            ...
            # Set the probability versions to None to save memory
            self._left_hippo_probs = None
            self._right_hippo_probs = None
...
def make_parser():
    ...
    # Add option for saving hippocampus probabilities
    parser.add_argument(
        "--save_hippo_probs",
        default=True,
        action='store_true',
        help="Save probability maps for hippocampus (left and right) as a separate file.",
    )
...
def main(
    ...
    # DEBUG: Print to verify our modified file is being used
    LOGGER.info("=== USING MODIFIED RUN_PREDICTION.PY WITH HIPPOCAMPUS PROBABILITY SAVING ===")
    LOGGER.info(f"save_hippo_probs parameter: {save_hippo_probs}")
    ...
    try:
        ...
        eval = RunModelOnData(
            ...
            save_hippo_probs=save_hippo_probs,
        )
        
        # Pass the save_hippo_probs flag to the eval object
        eval.save_hippo_probs = save_hippo_probs
    ...
    for subject, (orig_img, data_array) in iter_subjects:
        ...
        try:
            ...
            # Save hippocampus probability maps if requested
            if save_hippo_probs and hasattr(eval, '_left_hippo_raw') and hasattr(eval, '_right_hippo_raw'):
                # Create the hippocampus probability file paths
                segfile_path = Path(subject.segfile)
                left_hippo_raw_file = segfile_path.parent / (segfile_path.stem.replace('.nii', '') + '.left_hippo_raw.mgz')
                right_hippo_raw_file = segfile_path.parent / (segfile_path.stem.replace('.nii', '') + '.right_hippo_raw.mgz')
                
                LOGGER.info(f"Saving left hippocampus raw values to {left_hippo_raw_file}")
                LOGGER.info(f"Saving right hippocampus raw values to {right_hippo_raw_file}")
                
                # Save raw value versions for comparison
                futures.append(
                    eval.async_save_img(
                        str(left_hippo_raw_file), eval._left_hippo_raw, orig_img, dtype=np.float32
                    )
                )
                futures.append(
                    eval.async_save_img(
                        str(right_hippo_raw_file), eval._right_hippo_raw, orig_img, dtype=np.float32
                    )
                )
